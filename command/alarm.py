...
def set_alarm(minute, hour, day_of_month, month, comment):
    now = datetime.now()
    ...
    return f"Báo thức đã được đặt cho {alarm_time.strftime('%Y-%m-%d %H:%M')}."
...
def alarm_reminder_action(text):
    set_match = re.search(
        r'\b(?:đặt|tạo|lên lịch|báo thức|đánh thức tôi)\b.*?\b(?:lúc|trong|sau)?\s*(\d{1,2}:\d{2}|\d+\s*(?:phút|giờ))\b',        text, 
        re.IGNORECASE
    )
    delete_match = re.search(
        r'\b(?:xóa|hủy)\s+(?:một\s+)?báo\s+thức\b.*?\b(?:tên|gọi\s+là)\s*(\w+)', 
        text, 
        re.IGNORECASE
    )
    ...
    if set_match:
        ...
        if time_expression is None:
            return "Không xác định được thời gian cho báo thức."
        minute, hour, day, month = parse_time_expression(time_expression)
        comment = "Báo thức"
        return set_alarm(minute, hour, day, month, comment)
